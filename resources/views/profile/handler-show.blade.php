...
<x-layouts.app :title="'Handler Profile'">
    ...
    @include('partials.handler-sidebar')
        ...
        <div class="flex-1">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Handler Profile</h1>
                    <p class="text-sm text-gray-600">Manage your handler information and account settings</p>
                </div>
                <a href="{{ route('dashboard') }}" class="inline-flex items-center px-3 py-2 bg-emerald-600 rounded-lg text-white text-sm font-medium hover:bg-emerald-700">
                    Back to Dashboard
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="mt-6 text-sm space-y-2">
                        <div class="grid grid-cols-3 items-center">
                            ...
                            <div class="col-span-2 text-right">
                                @php($role = ucfirst(Session::get('user_role', 'handler')))
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-emerald-50 text-emerald-700">Handler</span>
                            </div>
                        </div>
                        <div class="grid grid-cols-3 items-center">
                ...
                {{-- Handler Profile Details / Forms (light only) --}}
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-2xl p-6 shadow-sm ring-1 ring-black/5">
                        ...
                        <form class="grid grid-cols-1 md:grid-cols-2 gap-4" method="POST" action="{{ route('profile.update') }}">
                            ...
                            <div>
                                ...
                                <input name="role" type="text" value="Handler" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm bg-gray-100 cursor-not-allowed focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" readonly tabindex="-1" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
                                <input name="full_name" type="text" value="{{ $pUser->name ?? Session::get('user_name', 'User') }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input name="department" type="text" value="{{ $profile->department ?? '' }}"
                                    class="w-full rounded-lg border px-3 py-2 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 {{ session('require_department') ? 'border-red-500 ring-2 ring-red-300 animate-pulse' : 'border-gray-300' }}" />
                                @if(session('require_department'))
                                    <div class="text-xs text-red-600 mt-1">Department is required. Please enter your department to continue.</div>
                                @endif
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Age</label>
                                <input name="age" type="number" min="1" max="120" value="{{ $profile->age ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Occupation</label>
                                <input name="occupation" type="text" value="{{ $profile->occupation ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nationality</label>
                                <input name="nationality" type="text" value="{{ $profile->nationality ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" />
                            </div>
                            <div class="md:col-span-1">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Contact Number</label>
                                <input name="contact_number" type="text" value="{{ $profile->contact_number ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" />
                            </div>
                            <div class="md:col-span-1">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Gender</label>
                                @php($gVal = $profile->gender ?? '')
                                <select name="gender" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input name="address" type="text" value="{{ $profile->address ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" />
                            </div>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm ring-1 ring-black/5">
                        ...
                        <form class="grid grid-cols-1 md:grid-cols-2 gap-4" onsubmit="event.preventDefault(); alert('Demo: not saving');">
                            <div class="col-span-2 md:col-span-1">
                                <label class="block text-sm font-medium text-gray-700 mb-1">New Password</label>
                                <input type="password" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
                            </div>
                            <div class="col-span-2 md:col-span-1">
                            ...
                            <div class="col-span-2 flex justify-end">
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-emerald-600 rounded-lg text-white text-sm font-medium hover:bg-emerald-700">Update Password</button>
                            </div>
                        </form>
                    </div>
                    @endif
